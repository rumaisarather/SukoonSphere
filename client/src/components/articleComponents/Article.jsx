...
const Article = ({ article }) => {
  const { title, author, datePublished, category, comments, views, image } = article;
  return (
    <div className=" grid grid-cols-1 sm:gap-4 border-b-2 items-center sm:grid-cols-3">
      ...
      <div className="sm:card-body sm:p-0 py-5 col-span-2 order-2 sm:order-none">
        <Link to="/articles/article">
          <h2 className="tranctcate-text-2 card-title col-span-2 text-[var(--primary)] hover:text-[var(--ternery)]">
            {title}
          </h2>
        </Link>
        <p className="tranctcate-text-5 text-[var(--black-color)]">
      ...
      {/* card image */}
      <div className="flex justify-between flex-col order-1 sm:order-none">
      ...
      <div className="flex items-center col-span-2 pb-4 justify-start gap-8 order-3 sm:order-none">
        ...
        <div className="flex items-center justify-center gap-2">
          ...
          <span className="text-sm text-[var(--primary)]"> {datePublished || "date"}</span>
        </div>
        <div className="hidden sm:flex items-center justify-between sm:justify-center gap-4">
          <div className="flex items-center justify-center gap-2">
            <AiOutlineComment color="var(--ternery)" />
            <span className="text-xs text-[var(--primary)]">{comments.length || "5"}</span>
          </div>
          |
          <div className=" flex items-center justify-center gap-2 ">
