          ...
          <ul className="flex mt-10 h-16 gap-5 items-center">
            {links.map((link) => (
              <div
                ...
                key={link.name}
              >
                <NavLink to={link.address} className="flex items-center">
                  {link.icon}
                  <span className="ml-1">{link.name}</span>
                </NavLink>

                {link.sublinks && (
                  <ul className="top-14 absolute opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-opacity duration-200 ease-in-out transform group-hover:translate-y-1 shadow-[0px_1px_10px_rgba(0,0,0,0.1)] bg-white p-2 rounded-[5px] w-48">
        ...
        {isUser ? (
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="hidden sm:flex bg-black items-center gap-1 rounded-[5px] transition-all ease-in-out duration-600 text-white px-3 py-2 text-xs leading-[1.32] hover:bg-[#f2f6ff]"
            >
              <span>Sign up</span>
              <FaArrowRightLong />
            </button>
